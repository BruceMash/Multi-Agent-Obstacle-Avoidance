...
def configure(folder: str | None = None, format_strings: list[str] | None = None) -> Logger:
    ...
    :param folder: the save location
        (if None, $BASELINE_LOGDIR, if still None, tempdir/BASELINE-[date & time])
    :param format_strings: the output logging format
        (if None, $BASELINE_LOG_FORMAT, if still None, ['stdout', 'log', 'csv'])
    :return: The logger object.
    """
    if folder is None:
        folder = os.getenv("BASELINE_LOGDIR")
    if folder is None:
        folder = os.path.join(tempfile.gettempdir(), datetime.datetime.now().strftime("BASELINE-%Y-%m-%d-%H-%M-%S-%f"))
    assert isinstance(folder, str)
    os.makedirs(folder, exist_ok=True)

    log_suffix = ""
    if format_strings is None:
        format_strings = os.getenv("BASELINE_LOG_FORMAT", "stdout,log,csv").split(",")
