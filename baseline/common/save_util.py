...
from baseline.common.type_aliases import TensorDict
from baseline.common.utils import get_device, get_system_info

BASELINE_VERSION_ENTRY = "_baseline_version"
BASELINE_VERSION = "local-baseline"
...
def save_to_zip_file(
    ...
    with zipfile.ZipFile(file, mode="w") as archive:
        ...
        # 保存当前本地 baseline 的版本标记，避免继续使用旧包名字段。
        archive.writestr(BASELINE_VERSION_ENTRY, BASELINE_VERSION)
        ...
        with zipfile.ZipFile(file) as archive:
            ...
            if print_system_info:
                if "system_info.txt" in namelist:
                    ...
                    warnings.warn(
                        "The model archive does not contain system information.",
                        UserWarning,
                    )
