...
from baseline.common.distributions import (
...
from baseline.common.preprocessing import get_action_dim, is_image_space, maybe_transpose, preprocess_obs
from baseline.common.torch_layers import (
...
from baseline.common.type_aliases import PyTorchObs, Schedule
from baseline.common.utils import get_device, is_vectorized_observation, obs_as_tensor
...
class BasePolicy(BaseModel, ABC):
    ...
    def predict(
        ...
        # Tuple obs are not supported by the current baseline, so we can safely do that check
        if isinstance(observation, tuple) and len(observation) == 2 and isinstance(observation[1], dict):
            raise ValueError(
                "You have passed a tuple to the predict() function instead of a Numpy array or a Dict. "
                "You are probably mixing Gym API with the baseline VecEnv API: `obs, info = env.reset()` (Gym) "
                "vs `obs = vec_env.reset()` (baseline VecEnv). "
...
class ActorCriticPolicy(BasePolicy):
    ...
    def __init__(
        ...
        if isinstance(net_arch, list) and len(net_arch) > 0 and isinstance(net_arch[0], dict):
            warnings.warn(
                (
                    "Shared layers in the mlp_extractor are no longer supported in the current baseline, "
    ...
    def _build(self, lr_schedule: Schedule) -> None:
        ...
        if self.ortho_init:
            ...
            if not self.share_features_extractor:
                # Note(antonin): keep results aligned with the reference implementation
