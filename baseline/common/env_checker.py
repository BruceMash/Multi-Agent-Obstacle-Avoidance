...
from baseline.common.preprocessing import check_for_nested_spaces, is_image_space_channels_first
from baseline.common.vec_env import DummyVecEnv, VecCheckNan
...
def _check_non_zero_start(space: spaces.Space, space_type: str = "observation", key: str = "") -> None:
    ...
    if isinstance(space, (spaces.Discrete, spaces.MultiDiscrete)) and not _starts_at_zero(space):
        ...
        warnings.warn(
            ...
            "is not supported by the current baseline. "
...
def _check_image_input(observation_space: spaces.Box, key: str = "") -> None:
    """
    Check that the input will be compatible with the current baseline
...
def _check_unsupported_spaces(env: gym.Env, observation_space: spaces.Space, action_space: spaces.Space) -> bool:  # noqa: C901
    """
    Emit warnings when the observation space or action space used is not supported by the current baseline.
    ...
    if isinstance(observation_space, spaces.Dict):
        ...
        if nested_dict:
            warnings.warn(
                "Nested observation spaces are not supported by the current baseline "
    ...
    if isinstance(observation_space, spaces.MultiDiscrete) and len(observation_space.nvec.shape) > 1:
        warnings.warn(
            ...
            "which is currently not supported by the current baseline. "
    ...
    if _is_oneof_space(observation_space):
        warnings.warn(
            "OneOf observation space is not supported by the current baseline. "
    ...
    if isinstance(observation_space, spaces.Sequence) or sequence_space:
        warnings.warn(
            "Sequence observation space is not supported by the current baseline. "
    ...
    if isinstance(observation_space, spaces.Graph) or graph_space:
        warnings.warn(
            "Graph observation space is not supported by the current baseline. "
            "Note: The checks for returned values are skipped."
        )
        should_skip = True
