...
from Envs.continuous_3d_waypoint_env_small_scale import UAVWaypoint3DMAPFEnv
...
def make_env(n_agents=5, max_steps=200):
...
def train(args):
    ...
    env = make_env(n_agents=args.n_agents, max_steps=args.max_steps)
    eval_env = make_env(n_agents=args.n_agents, max_steps=args.max_steps)
    ...
    eval_callback = EvalCallback(
        ...
        eval_freq=max(1, 10000 // args.n_agents),
        deterministic=True,
        render=False,
    )
...
def evaluate(args):
    ...
    try:
        from test_sb3_waypoint_smallscale import plot_waypoint_env, announce_event
    except ImportError:
        print("未找到 test_sb3_waypoint.py，无法执行可视化评估。")
        return
    ...
    history = {agent: [] for agent in raw_env.possible_agents}
